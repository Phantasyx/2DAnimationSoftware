...
namespace Testing
{
	TEST_CLASS(CActorTest)
	{
		...
		TEST_METHOD(TestCActorEnabled)
		{
			...
			// test getter
			wstring test = actor.GetName();
			Assert::IsTrue(test == L"Harold");
		}
		...
		TEST_METHOD(TestCActorPosition)
		{
			...
			actor.SetPosition(Gdiplus::Point(800, 600));
			Assert::AreEqual(800, actor.GetPosition().X);
			Assert::AreEqual(600, actor.GetPosition().Y);
		}
